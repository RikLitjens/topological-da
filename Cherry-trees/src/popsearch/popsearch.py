...
class PopSearch:
    ...
    def initialize_population(self):
        for _ in range(self.K):
            self.skeletons_k.append(
                Skeleton(self.superpoints, self.raw_edges, self.base_node)
            )
    ...
    def create_next_gen(self):
        ...
        while len(chosen) != self.K:
            chosen = random.choices(
                candidate_pairs, weights=probabilities, k=self.K - len(chosen)
            )
    ...
    def get_weights(self):
        ...
        """
        weights = {}
        ranks_skeleton = create_rank_dict(
            lambda skel: skel.get_skel_score(), self.skeletons_k
        )
