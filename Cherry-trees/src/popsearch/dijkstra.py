...
class Dijkstra:
    def __init__(self, nodes, edges) -> None:
        ...
        # Initialize dijkstra
        weights = {
            (i, j): None for i in range(len(self.nodes)) for j in range(len(self.nodes))
        }

        #
        for edge in edges:
            start_index = self.node_index_map[edge.p1]
            end_index = self.node_index_map[edge.p2]

            weights[(start_index, end_index)] = edge.get_dijkstra_weight(
                predecessor=self.all_edge_collection.predecessor_map[edge.p1][0]
            )
    ...
    def dijkstra(self, graph_weights, start, candidate_edge):
        ...
        if (
            D[self.superpoints_idx_map[candidate_edge.p1]]
            > D[self.superpoints_idx_map[candidate_edge.p2]]
        ):
            # loop through predecessors to find all edges
            current_pre_node = candidate_edge.p2
            min_dist = D[self.superpoints_idx_map[candidate_edge.p2]]

        # shortest path to edge endpoint 2
        else:
            current_pre_node = candidate_edge.p1
            min_dist = D[self.superpoints_idx_map[candidate_edge.p1]]

        # get predecessors (Dijk in paper)
        dijk = []
        while dijk_predecessors[current_pre_node] is not None:
            # add edge that belongs to it
            predecessor_edge = self.all_edge_collection.point_edge_map[
